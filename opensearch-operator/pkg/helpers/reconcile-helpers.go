...
func ResolveControllerSidecarImage(nodePool *opsterv1.NodePool) (result opsterv1.ImageSpec) {
	defaultRepo := "docker.io/opensearchproject"
	defaultImage := "opensearch-operator-sidecar"
	defaultVersion := "latest"

	// if image is specified in nodePool, use that
	if useCustomImage(nodePool.ControllerSidecar.ImageSpec, &result) {
		return
	}

	// if image is specified via envvar (from helm chart), use that
	image, exists := os.LookupEnv("CONTROLLER_SIDECAR_IMAGE")
	if exists && image != "" {
		result.Image = &image
		return
	}

	version, exists := os.LookupEnv("CONTROLLER_SIDECAR_VERSION")
	if exists && version != "" {
